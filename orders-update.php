  ...
  foreach($_SESSION['cart'] as $product_id => $Quantity) {
    ...
    if($result){
      ...
      if($obj = $result->fetch_object()) {
        ...
        $query = $mysqli->query("INSERT INTO orders (productCode, productName, productDesc, price, units, total, email, username) VALUES('$obj->productCode', '$obj->productName', '$obj->productDesc', $obj->price, $Quantity, $cost, '$email', '$user')");

        if($query){
          $newqty = $obj->Quantity - $Quantity;
          if($mysqli->query("UPDATE products SET Quantity = ".$newqty." WHERE id = ".$product_id)){
